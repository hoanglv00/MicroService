...
function insertId(response) {
    return Promise.resolve(lodash.merge(response.data, { departmentId: 1 }));
}
...
function compareKPIDepartmentByMonth(req, res, next) {
...
function getKPIPerTagertByMonth(req, res, next) {
    // var urls = ['http://18.191.133.197:8083/api/v1/departmentKPI/getDepartmentKPI?'
    //     + setUrlForGetList(req) + '&' + 'departmentId=' + req.query.departmentId,
    //     'http://18.191.133.197:8083/api/v1/departmentKPI/getDepartmentTargetKPI?'
    //     + setUrlForGetList(req) + '&' + 'departmentId=' + req.query.departmentId];
    // console.log(urls);
    // Promise.all(urls.map(url =>
    //     fetch(url)
    ...
    res.status(200)
        .json({
            status: 'success',
            data: '{KPI value: 80, KPI target: 100, %: 80}'
        });
    // })
}

function getListKPIDepartmentByMonth(req, res, next) {
...
module.exports = {
    compareKPIDepartmentByMonth: compareKPIDepartmentByMonth,
    getListKPIDepartmentByMonth: getListKPIDepartmentByMonth,
    getKPIPerTagertByMonth: getKPIPerTagertByMonth
}
